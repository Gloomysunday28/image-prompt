...
3. 分清模板里哪些是**风格身份**、哪些是**画面内容**。这是最关键的一步：

   - **锁死（风格身份）**：媒介与质感、色彩关系、光线的物理逻辑、描述精度、禁止清单。动了这些，出来的图就不再属于这个模板。
   - **必须变（画面内容）**：发生了什么事、谁在场、在做什么、哪个时刻、什么天气、从哪个位置看、主体是谁、占多大。**机位、焦段、构图属于内容，不是身份**——同一种画风完全可以换个位置拍。

   另外**保留至少一个模板的招牌视觉元素**作为血缘证据（治愈那套的电车 / 宝石般的海湾 / 冲天积雨云，任选其一即可），其余都可以换掉。

4. **改动下限：一眼可见。** 把你的成品和模板并排，一个只看过模板的人必须立刻认出这是**新的一张**，而不是同一张的微调。只加一两个小道具、只挪一下人物位置、只换个天气词，都不算。
...
- 每份 Prompt 前用**一句话**说清：这次和模板的差别是什么，以及保留了哪个招牌元素。说不出实质差别，就说明这份不该交。
- 成组图：共享设定逐字一致，只让一条变量变化。
- 直接给成品，不留占位符，不夹解释。

## 自检

- **把成品和模板并排，认得出这是新的一张吗？** 认不出就是失败，重做。
- 有没有整段照抄模板的句子？
- 锁死的是风格身份，变的是画面内容——我有没有搞反（比如锁了机位却去动色彩）？

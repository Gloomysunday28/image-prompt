...
它**唯一的风格来源就是本仓库里的模板文件**，没有任何预设的风格表或场景池。它会先从分类中选择一份主模板，判断这是完整场景、世界观设定、角色设定、参数模板还是短风格参考，再提取模板的视觉 DNA：媒介、世界身份、角色与标志物、空间关系、材质逻辑、光色和禁止项。

默认采用“同世界续拍”：保留真正定义模板的角色、世界和构图关系，改变情境、时刻、天气、动作或局部地点。只有明确要求换主体或换题材时，才进行风格迁移，并把原模板中的尺度参照、引导线、界面结构等按视觉功能映射到新题材，而不是机械地删光所有具体元素。同一目录里的文件也不会被默认混读；只有明确属于同一系列或能作为兼容设定块时才会组合。

关键约定是**模板即定义，分类不是定义**。例如 [`治愈/夏日海边.md`](治愈/夏日海边.md) 是梦幻真实摄影，明确禁止插画感与动漫感；[`治愈/滑板治愈.md`](治愈/滑板治愈.md) 则是日系动画电影背景美术与高质量 3D CGI / 写实摄影融合。用户只说「治愈系」时，技能会先区分这两条路线并选择一份主模板，而不是把整个目录概括成一种通俗画风。其他分类也遵循同一规则。
